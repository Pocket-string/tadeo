      ...
      if (openTrades && openTrades.length > 0) {
        ...
        if (currentATR && !trade.metadata?.breakeven_hit) {
          ...
          if (shouldActivateBE) {
            ...
            results.push({
              ...
              riskTier,
              action: 'trail',
              reason: `Breakeven activated: SL moved to entry ${entryPrice.toFixed(2)}`,
              price,
            })
            continue
          }
        ...
        if (currentATR && trade.metadata?.breakeven_hit) {
          ...
          if (params.trailing_stop_mode === 'ema') {
            ...
            // Old: 15%→30%, 40%→50%, 70%→70% — too aggressive, choked trades at +0.13 ATR avg
            let lockPct: number
...
// EMA trail cache — keyed by `symbol:timeframe:period` — reset each POST invocation
...
async function checkSignalFull(
  ...
  if (htfBias === 'bear' && signal === 'buy') return { signal: null, rejectionReason: `bear_regime_pause:bias=bear,signal=buy` }
