...
export async function POST(req: NextRequest) {
  ...
  for (const session of sessions) {
    ...
    try {
      ...
      const estimatedFee = price * finalQuantity * 0.002 // 0.1% per side roundtrip
      const potentialReward = currentATR * tpMult * finalQuantity
      if (potentialReward > 0 && estimatedFee / potentialReward > 0.20) {
        await supabase.from('live_agent_log').insert({
        ...
        results.push({
          sessionId: session.id, symbol: session.symbol, timeframe: session.timeframe, riskTier,
          action: 'skip', reason: `Fee ratio ${(estimatedFee / potentialReward * 100).toFixed(1)}% exceeds 20% cap`, price,
        })
        continue
      }
